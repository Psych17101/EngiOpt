...
class Args:
    ...
    n_cpu: int = 8
    latent_dim: int = 100  # c ~ uniform
    noise_dim: int = 6  # z ~ normal
...
if __name__ == "__main__":
    ...
    disc = Discriminator(latent_dim=args.latent_dim).to(device)
    ...
    d_optimizer = th.optim.Adam(disc.parameters(), lr=0.00005, betas=(args.b1, args.b2), eps=_EPS)
    g_optimizer = th.optim.Adam(generator.parameters(), lr=0.0002, betas=(args.b1, args.b2), eps=_EPS)
    ...
    for epoch in range(args.n_epochs):
        for i, real_designs_cpu in enumerate(dataloader):
            ...
            logits_real, _ = disc(real_designs)
            ...
            logits_fake2, q_out2 = disc(x_fake2)
