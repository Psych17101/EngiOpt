...
def train_one_model(
    ...
    Args:
        args: Training arguments. This should include attributes like:
              ...
              lr_decay_step, patience, have_shape_cols, shape_dim, num_cont, num_cat,
    ...
    """
    # Extract additional configuration from args (with defaults)
    have_shape_cols = getattr(args, "have_shape_cols", False)
    shape_dim = getattr(args, "shape_dim", 0)
    num_cont = getattr(args, "num_cont", 0)
    num_cat = getattr(args, "num_cat", 0)

    from vae_model import shape2shape_loss
    from vae_model import Shape2ShapeVAE
    ...
    if have_shape_cols and args.structured:
        ...
        def train_step(batch_data: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        def valid_step(batch_data: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
            x_init_batch, x_opt_batch, param_batch, cl_batch = batch_data
            x_init_batch = x_init_batch.to(device)
            x_opt_batch = x_opt_batch.to(device)
            param_batch = param_batch.to(device)
