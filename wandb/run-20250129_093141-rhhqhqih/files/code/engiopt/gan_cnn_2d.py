...
class Generator(nn.Module):
    def __init__(
        ...
        # We'll map the latent vector z into a (g_features*4) x 25 x 25 feature map
        self.init_size = 25  # We will end up with 25 x 25 first
        self.fc = nn.Linear(latent_dim, g_features * 4 * self.init_size * self.init_size)

        # Upsampling layers:
        # 1) 25 -> 50
        # 2) 50 -> 100
        # 3) Kernel 3 (stride=1) to keep 100 -> 100 but adjust channels to out_channels
        self.conv_blocks = nn.Sequential(
            nn.ConvTranspose2d(
                g_features * 4,      # in_channels
                g_features * 2,      # out_channels
                kernel_size=4,
                stride=2,
                padding=1,
                bias=False
            ...
            nn.Tanh()  # final activation for GAN
        )

    def forward(self, z):
        # Map latent vector z to the initial 2D feature map
        out = self.fc(z)  # shape: (batch_size, g_features*4*25*25)
        out = out.view(out.size(0), -1, self.init_size, self.init_size) 
        img = self.conv_blocks(out)  # shape: (batch_size, out_channels, 100, 100)
        return img
...
class Discriminator(nn.Module):
    def __init__(
        ...
        # Convolutional blocks that downsample from 100 -> 50 -> 25 -> 13 -> 7 -> 1
        # You may have to carefully check the shapes to ensure you get to a 1×1 at the end.
        # The formula for output size of Conv2d is:
        ...
        self.conv_blocks = nn.Sequential(
            ...
            # Note: at this point, shape is (batch_size, 1, 1, 1)
        )

        self.output_act = nn.Sigmoid()

    def forward(self, img):
        # img shape: (batch_size, in_channels, 100, 100)
        out = self.conv_blocks(img)  
        # out shape: (batch_size, 1, 1, 1)
        out = out.view(out.size(0), -1)  # shape => (batch_size, 1)
